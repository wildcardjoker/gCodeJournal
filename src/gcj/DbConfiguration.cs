...
{
    static IConfiguration ApplyEfLoggingOverride(IConfiguration config, bool enableQueryLogging)
    {
    ...
    static IConfiguration BuildConfiguration() => new ConfigurationBuilder().AddJsonFile("appsettings.json", false, true).Build();

    static DbContextOptionsBuilder<GCodeJournalDbContext> ConfigureDbContextOptions(string dbPath, ILoggerFactory loggerFactory) =>
        new DbContextOptionsBuilder<GCodeJournalDbContext>()
    ...
    static LoggerConfiguration CreateLoggerConfiguration(IConfiguration config)
    {
    ...
    static ILoggerFactory CreateLoggerFactory(bool enableQueryLogging) =>
    ...
    static string EnsureLogFilePath(IConfiguration config)
    {
        var writeTos    = config.GetSection("Serilog:WriteTo").GetChildren().ToList();
